...
const ChatItem = ({ chat, connectedUsers }) => {
  ...
  return (
    <li
      ...
      onClick={() =>
        router.push(`/messages?chat=${chat.messagesWith}`, {
          shallow: true,
        })
      }
    ...
    >
      <button className="flex items-center w-full px-4 py-2 select-none hover:bg-gray-100 focus:outline-none">
        <div className="relative">
          ...
          />
          <div
            className={`absolute bottom-1.5 right-0 rounded-full h-4 w-4 border-2 border-white ${isOnline ? 'bg-green-400' : 'bg-red-400'
              }`}
          ></div>
        </div>
        <div className="transform translate-y-0.5 ml-3 text-left">
          <h3 className="leading-4">
            {chat.name}{' '}
            <span className="text-xs text-grayelit font-inconsolata">
              {format(new Date(chat.date), 'dd MMM, HH:mm', { locale: ptBR })}
            </span>
          </h3>
          <span className="text-xs font-inconsolata text-deepgrayelit">
